...
manage this problem is to develop a script that would first inventory my music catalog.  Then, I could set up specific selection criteria for the songs
...
to listen to and copy those songs to my flash drive.  This script represents that effort.

Note: between the large amount of files I have and invoking the COM object shell.application, this script does take a very long time to run.  
...
$flashdrive_location = "N:\"  # set the path to your flash drive
$mp3_collection = @()
...
# now, I need to decide what songs I copy over to my flash drive for listening in the car; selecting on genre seems to be a pretty good bet
$genres_for_car = "Metal", "Hard Rock & Metal", "Rock", "Rock; Hard Rock & Metal"
$mp3_collection | where {$genres_for_car -contains $_.genre} | select file | % {Copy-Item $_.file -Destination $flashdrive_location}
